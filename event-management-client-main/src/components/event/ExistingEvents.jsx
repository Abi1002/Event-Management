...
const ExistingEvents = () => {
	...
	const [filteredEvents, setFilteredEvents] = useState([{ id: "", eventType: "", eventPrice: "" }])
	...
	useEffect(() => {
		if (selectedEventType === "") {
			...
			const filteredEvents = events.filter((event) => event.eventType === selectedEventType)
			setFilteredREvents(filteredEvents)
		}
		setCurrentPage(1)
	...
	const handleDelete = async (eventId) => {
		try {
			...
			if (result === "") {
				setSuccessMessage(`Event No ${eventId} was deleted`)
				fetchEvents()
			} else {
